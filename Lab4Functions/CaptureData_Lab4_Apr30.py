...
class ObservationPoint:
    ...
    dec: float
    is_calibration: bool
    mode: Literal["grid", "track", "specific"]
...
def precompute_observation_plan(mode="grid", num_points=300):
    ...
    if mode == "grid":
        ...
        raw_points = []
        for l, b in specific_coords:
            ra, dec = galactic_to_equatorial(l, b)
            raw_points.append(ObservationPoint(id=id_counter, gal_l=l, gal_b=b, ra=ra, dec=dec, is_calibration=False, mode="specific"))
            id_counter += 1
        sorted_counter, with_cal = 0, []
        for i, p in enumerate(raw_points):
            p.id = sorted_counter
            with_cal.append(p)
            if (i + 1) % CAL_INTERVAL == 0:
                cal_p = ObservationPoint(id=p.id, gal_l=p.gal_l, gal_b=p.gal_b, ra=p.ra, dec=p.dec, is_calibration=True, mode="specific")
                with_cal.append(cal_p)
            sorted_counter += 1
        return sorted(with_cal, key=lambda p: (p.ra, p.dec))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", choices=["grid", "track", "specific"], default="grid")
